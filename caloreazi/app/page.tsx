  ...
  return (
    <main className={dark ? "app-shell theme-dark" : "app-shell"} dir={profile?.language === "en" ? "ltr" : "rtl"} lang={profile?.language || "he"}>
      ...
      <section className="today-focus-card" aria-label="סיכום היום בקצרה">
        ...
        <button type="button" onClick={() => { const hour = new Date().getHours(); setSuggestionPeriod(hour < 11 ? "breakfast" : hour < 16 ? "lunch" : hour < 20 ? "dinner" : "snack"); window.setTimeout(() => document.getElementById("meal-suggestions")?.scrollIntoView({ behavior: "smooth", block: "start", inline: "nearest" }), 0); }}>הצג 3 הצעות</button>
      </section>
      ...
      {cameraCaptureOpen && <div className="modal-layer meal-camera-layer"><button className="backdrop" type="button" onClick={closeInAppCamera} aria-label="סגירת המצלמה" /><button className="camera-floating-exit" type="button" onClick={closeInAppCamera} aria-label="יציאה ממצב צילום"><span aria-hidden="true">×</span><b>יציאה</b></button><section className="settings-modal meal-camera-modal"><header><div><h2>צילום ארוחה</h2><p>מקם את כל הצלחת בפריים ובתאורה טובה</p></div></header><div className="live-meal-camera"><video ref={mealCameraVideo} playsInline muted /><span><AppIcon name="target" /></span></div><p className="camera-status" role="status">{cameraStatus}</p><label>פרט שיעזור בזיהוי <input value={cameraHint} onChange={(event) => setCameraHint(event.target.value)} maxLength={300} placeholder="לא חובה — למשל: חזה עוף עם מעט שמן" /></label><footer><button type="button" onClick={() => { closeInAppCamera(); uploadInput.current?.click(); }}>בחר מהגלריה</button><button className="primary camera-shutter" type="button" onClick={captureInAppMeal} aria-label="צלם ונתח"><AppIcon name="camera" /> צלם ונתח</button></footer></section></div>}
      ...
      {barcodeScannerOpen && <div className="modal-layer barcode-scanner-layer"><button className="backdrop" onClick={() => setBarcodeScannerOpen(false)} /><button className="camera-floating-exit" type="button" onClick={() => setBarcodeScannerOpen(false)} aria-label="יציאה מסריקת ברקוד"><span aria-hidden="true">×</span><b>יציאה</b></button><section className="barcode-scanner"><header><div><strong>סריקת ברקוד</strong><small>המצלמה מנתחת וידאו מקומית בלבד</small></div></header><div className="barcode-video-frame"><video ref={barcodeVideo} playsInline muted /><i /><span>מקם את הברקוד בתוך המסגרת</span></div><p>{barcodeStatus}</p><button type="button" onClick={() => setBarcodeScannerOpen(false)}>ביטול וחזרה להוספת ארוחה</button></section></div>}
      {customFoodOpen && (
        <div className="modal-layer">
          <button
            className="backdrop"
