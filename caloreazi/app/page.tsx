...
export default function Home() {
  ...
  return (
    <main className={dark ? "app-shell theme-dark" : "app-shell"} dir="rtl">
      ...
      {historyOpen && <div className="modal-layer"><button className="backdrop" onClick={() => setHistoryOpen(false)} /><section className="settings-modal history-modal"><header><div><p className="eyebrow">יומן תזונה</p><h2>ציר הזמן שלי</h2></div><button onClick={() => setHistoryOpen(false)}>×</button></header><div className="history-days">{[...(state.history || []), state.today].sort((a, b) => b.date.localeCompare(a.date)).map((day) => { const totals = day.meals.reduce((sum: any, meal: any) => ({ kcal: sum.kcal + Number(meal.kcal || 0), protein: sum.protein + Number(meal.protein || 0), carbs: sum.carbs + Number(meal.carbs || 0), fat: sum.fat + Number(meal.fat || 0) }), { kcal: 0, protein: 0, carbs: 0, fat: 0 }); return <details key={day.date} open={day.date === state.today.date}><summary><div><strong>{new Date(`${day.date}T12:00:00`).toLocaleDateString("he-IL", { weekday: "long", day: "numeric", month: "long" })}</strong><small>{day.meals.length} ארוחות · {day.waterMl || 0} מ״ל מים</small></div><b>{totals.kcal.toLocaleString()} kcal</b></summary><div className="history-summary"><span>חלבון <b>{totals.protein}g</b></span><span>פחמימות <b>{totals.carbs}g</b></span><span>שומן <b>{totals.fat}g</b></span></div><section className="history-timeline">{[...day.meals].sort((a: any, b: any) => String(a.time).localeCompare(String(b.time))).map((meal: any) => <article className={`timeline-${meal.period || "snack"}`} key={meal.id}><time>{new Date(meal.time).toLocaleTimeString("he-IL", { hour: "2-digit", minute: "2-digit" })}</time><i />{meal.image ? <img src={meal.image} alt="" /> : <span className="timeline-icon">🍽</span>}<div><em>{periodLabels[meal.period || "snack"]}</em><strong>{meal.name}</strong><small>{meal.protein}g חלבון · {meal.carbs}g פחמימות · {meal.fat}g שומן</small></div><b>{meal.kcal}<small> kcal</small></b></article>)}</section></details>; })}</div></section></div>}
