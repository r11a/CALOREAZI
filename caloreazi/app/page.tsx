...
export default function Home() {
  ...
  async function startVoiceRecording() {
    ...
    try {
      ...
      const recorder = supportedType ? new MediaRecorder(stream, { mimeType: supportedType }) : new MediaRecorder(stream); mediaRecorder.current = recorder; audioChunks.current = [];
      ...
      recorder.onstop = async () => {
        ...
        if (blob.size < 500) { setVoiceStatus("ההקלטה קצרה מדי או ריקה. נסה לדבר לפחות שתי שניות."); return; }
  ...
  function stopVoiceRecording() { if (mediaRecorder.current?.state === "recording") { mediaRecorder.current.requestData(); mediaRecorder.current.stop(); } setRecording(false); }
