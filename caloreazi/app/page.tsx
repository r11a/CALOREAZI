...
export default function Home() {
  ...
  async function addMeal(event: FormEvent) {
    ...
    try {
      ...
      setMealPeriod("snack"); setManualDescription(""); setFoodCategory("meals"); setManualAiMode(false);
    } catch (e) {
      setError((e as Error).message);
    } finally {
      setBusy(false);
  ...
  function openManualMeal(category = "meals") { setMealForm({ name: "", kcal: 0, protein: 0, carbs: 0, fat: 0 }); setMealItems([]); setAiOriginalItems([]); setMealSource("manual"); setManualAiMode(true); setManualDescription(""); setFoodCategory(category); setPhotoPreview(""); setPhotoStatus("תאר את הארוחה וה-AI יחשב ויפרק אותה לפריטים לפני האישור."); setQuickAddOpen(false); setMealOpen(true); }
      ...
      {mealOpen && (
        <div className="modal-layer">
          ...
          <form className="settings-modal meal-modal" onSubmit={addMeal}>
            ...
            {manualAiMode && mealItems.length === 0 && <section className="manual-ai-entry"><label>מה אכלת?<textarea rows={3} value={manualDescription} onChange={(e) => setManualDescription(e.target.value)} placeholder="למשל: שתי חתיכות פרגית, כוס אורז וסלט קטן" /></label><button type="button" onClick={analyzeManualDescription} disabled={busy || !manualDescription.trim()}>{busy ? "מחשב…" : "חשב אוטומטית עם AI"}</button><small>לאחר החישוב יוצגו כל הפריטים, הכמויות וההנחות לעריכה ולאישור.</small></section>}
            <div className="settings-grid">
