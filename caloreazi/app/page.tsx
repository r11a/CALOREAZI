...
const quickFoods: Record<string, { name: string; icon: string; portion: string; kcal: number; protein: number; carbs: number; fat: number }[]> = {
  vegetables: [
    ...
    { name: "קישוא", icon: "🥒", portion: "קישוא בינוני", kcal: 33, protein: 2, carbs: 6, fat: 1 }, { name: "חציל", icon: "🍆", portion: "חציל בינוני", kcal: 114, protein: 5, carbs: 27, fat: 1 }, { name: "כרובית", icon: "🥬", portion: "כוס מבושלת", kcal: 29, protein: 2, carbs: 5, fat: 1 }, { name: "בטטה", icon: "🍠", portion: "בטטה בינונית", kcal: 112, protein: 2, carbs: 26, fat: 0 },
  ],
  fruits: [
    ...
    { name: "מנגו", icon: "🥭", portion: "כוס חתוכה", kcal: 99, protein: 1, carbs: 25, fat: 1 }, { name: "שזיף", icon: "🟣", portion: "שזיף בינוני", kcal: 30, protein: 0, carbs: 8, fat: 0 }, { name: "מלון", icon: "🍈", portion: "כוס חתוכה", kcal: 54, protein: 1, carbs: 13, fat: 0 }, { name: "אשכולית", icon: "🍊", portion: "חצי אשכולית", kcal: 52, protein: 1, carbs: 13, fat: 0 }, { name: "אבטיח", icon: "🍉", portion: "שתי כוסות חתוכות", kcal: 92, protein: 2, carbs: 23, fat: 0 }, { name: "קיווי", icon: "🥝", portion: "שני פירות", kcal: 84, protein: 2, carbs: 20, fat: 1 }, { name: "אפרסק", icon: "🍑", portion: "אפרסק בינוני", kcal: 59, protein: 1, carbs: 14, fat: 0 },
  ],
  drinks: [
...
function foodSpriteStyle(category: string, index: number) {
  const layout = category === "fruits" ? { columns: 5, rows: 3, file: "food-sprite-fruits-v4.webp" } : category === "vegetables" ? { columns: 4, rows: 3, file: "food-sprite-vegetables-v4.webp" } : { columns: 4, rows: 2, file: "food-sprite-drinks-v3.webp" };
  const column = index % layout.columns; const row = Math.floor(index / layout.columns);
  return { backgroundImage: `url(${layout.file})`, backgroundSize: `${layout.columns * 100}% ${layout.rows * 100}%`, backgroundPosition: `${layout.columns === 1 ? 0 : column * 100 / (layout.columns - 1)}% ${layout.rows === 1 ? 0 : row * 100 / (layout.rows - 1)}%` };
}

function goalStatus(value: number, target: number) {
  if (!target) return { className: "goal-unknown", label: "אין יעד" };
  const ratio = value / target;
  if (ratio < .9) return { className: "goal-under", label: "מתחת ליעד" };
  if (ratio < .98) return { className: "goal-near", label: "קרוב ליעד" };
...
async function api(url: string, options?: RequestInit) {
  ...
  const response = await fetch(target, {
    ...options,
    cache: "no-store",
    headers: multipart ? options?.headers : {
...
export default function Home() {
  ...
  const [mealDateTime, setMealDateTime] = useState(() => { const date = new Date(); date.setMinutes(date.getMinutes() - date.getTimezoneOffset()); return date.toISOString().slice(0, 16); });
  ...
  const [activityForm, setActivityForm] = useState({ type: "הליכה", minutes: 30, steps: 0, distanceKm: 0, activeCalories: 0 });
  ...
  const voiceProcessingTimer = useRef<ReturnType<typeof setInterval> | null>(null);
  ...
  useEffect(() => { const refresh = () => { if (document.visibilityState === "visible") api("/api/state").then(setState).catch(() => undefined); }; window.addEventListener("focus", refresh); document.addEventListener("visibilitychange", refresh); return () => { window.removeEventListener("focus", refresh); document.removeEventListener("visibilitychange", refresh); }; }, []);
  ...
  async function restoreTrash(id: string) { try { setState(await api("/api/trash", { method: "PATCH", body: JSON.stringify({ id }) })); setTrashItems((items) => items.filter((item) => item.id !== id)); } catch (e) { setError((e as Error).message); } }
  async function addMeal(event: FormEvent) {
    ...
    try {
      ...
      if (saveToLibrary || catalogOnly) {
        await api("/api/foods", { method: "POST", body: JSON.stringify({ ...finalMeal, category: foodCategory, visibility: foodVisibility, image: generateFoodArtwork ? "" : photoPreview, generateImage: generateFoodArtwork }) });
  ...
  async function deleteMyData() { const confirmation = window.prompt("מחיקה זו מסירה לצמיתות את החשבון, הארוחות, המדידות, השיחות והתמונות. כדי להמשיך הקלד DELETE MY DATA"); if (confirmation !== "DELETE MY DATA") return; setBusy(true); try { await api("/api/export", { method: "DELETE", body: JSON.stringify({ confirmation }) }); window.location.reload(); } catch (e) { setError((e as Error).message); setBusy(false); } }
  ...
  async function saveEditedFood(event: FormEvent) { event.preventDefault(); if (!editingFood) return; setBusy(true); try { await api("/api/foods", { method: "PATCH", body: JSON.stringify(editingFood) }); setState(await api("/api/state")); setEditingFood(null); } catch (e) { setError((e as Error).message); } finally { setBusy(false); } }
  async function removeCatalogFood(food: any) { if (!window.confirm(`להסיר את ${food.name} מספריית המאכלים?`)) return; setBusy(true); try { await api("/api/foods", { method: "DELETE", body: JSON.stringify({ id: food.id }) }); setState(await api("/api/state")); if (editingFood?.id === food.id) setEditingFood(null); } catch (e) { setError((e as Error).message); } finally { setBusy(false); } }
  ...
  async function startVoiceRecording() {
    ...
    try {
      ...
      const recorder = new MediaRecorder(stream); mediaRecorder.current = recorder; audioChunks.current = []; browserTranscript.current = "";
      const Recognition = (window as any).SpeechRecognition || (window as any).webkitSpeechRecognition;
      if (Recognition) { const recognition = new Recognition(); recognition.lang = "he-IL"; recognition.continuous = true; recognition.onresult = (event: any) => { browserTranscript.current = Array.from(event.results).map((result: any) => result[0]?.transcript || "").join(" ").trim(); }; speechRecognition.current = recognition; try { recognition.start(); } catch { speechRecognition.current = null; } }
      recorder.ondataavailable = (event) => { if (event.data.size) audioChunks.current.push(event.data); };
      recorder.onerror = () => { stream.getTracks().forEach((track) => track.stop()); speechRecognition.current?.stop(); speechRecognition.current = null; if (voiceTimer.current) clearInterval(voiceTimer.current); voiceTimer.current = null; setRecording(false); setVoiceSeconds(0); setVoiceStatus("הדפדפן הפסיק את ההקלטה. בדוק הרשאת מיקרופון ונסה שוב."); };
      recorder.onstop = async () => {
        if (voiceTimer.current) clearInterval(voiceTimer.current); voiceTimer.current = null; setVoiceSeconds(0);
        stream.getTracks().forEach((track) => track.stop());
        speechRecognition.current?.stop(); speechRecognition.current = null;
        const mimeType = audioChunks.current.find((chunk) => chunk.type)?.type || recorder.mimeType || "audio/webm";
        const blob = new Blob(audioChunks.current, { type: mimeType });
        mediaRecorder.current = null;
        if (blob.size < 800 || Date.now() - recordingStartedAt.current < 900) { setVoiceStatus("לא התקבלה הקלטה מספקת. החזק לפחות שתי שניות, דבר ואז לחץ עצור."); return; }
        const form = new FormData(); form.set("audio", blob, `meal-recording.${mimeType.includes("mp4") ? "m4a" : mimeType.includes("ogg") ? "ogg" : "webm"}`); form.set("browserTranscript", browserTranscript.current);
        setBusy(true); setVoiceProcessingSeconds(0); setVoiceStatus(browserTranscript.current ? "התמלול התקבל. מנתח פריטים וכמויות…" : "מעלה ומתמלל את ההקלטה…");
        voiceProcessingTimer.current = setInterval(() => setVoiceProcessingSeconds((seconds) => seconds + 1), 1000);
        try {
          const result = await api("/api/ai/analyze-voice", { method: "POST", body: form });
        ...
        } catch (e) { setVoiceStatus((e as Error).message); } finally { if (voiceProcessingTimer.current) clearInterval(voiceProcessingTimer.current); voiceProcessingTimer.current = null; setBusy(false); }
      };
      recordingStartedAt.current = Date.now(); recorder.start(); setVoiceSeconds(0); setRecording(true); setVoiceStatus("מקליט… דבר ברצף לפחות שתי שניות ואז לחץ עצור.");
      voiceTimer.current = setInterval(() => setVoiceSeconds((seconds) => { if (seconds >= 59) { if (recorder.state === "recording") recorder.stop(); setRecording(false); return 60; } return seconds + 1; }), 1000);
    } catch (error) { setVoiceStatus(error instanceof DOMException && error.name === "NotAllowedError" ? "לא ניתנה הרשאה למיקרופון. אשר גישה בדפדפן ונסה שוב." : "לא ניתן להפעיל את המיקרופון בדפדפן הזה. נסה לפתוח ב־HTTPS או בדפדפן חיצוני."); }
  }
  function stopVoiceRecording() { const recorder = mediaRecorder.current; if (recorder?.state === "recording") { setVoiceStatus("מסיים את ההקלטה…"); recorder.stop(); } setRecording(false); }
  ...
  return (
    <main className={dark ? "app-shell theme-dark" : "app-shell"} dir="rtl">
      ...
      <section className="primary-actions action-trio">
        <button className="camera-action" onClick={() => setCameraChoiceOpen(true)} disabled={busy}>
        ...
        <div className="panel meals-panel">
          ...
          {state.today.meals.length === 0 ? (
            ...
            <div className="meal-list">
              {[...state.today.meals].sort((a, b) => String(a.time).localeCompare(String(b.time))).map((meal) => (
                <article key={meal.id}>
                  {meal.image ? <img className="meal-thumb" src={meal.image} alt="" /> : <span className={`meal-icon meal-${meal.period || "snack"}`}>🍽</span>}
                  <div>
                    <span className="meal-meta"><time>{new Date(meal.time).toLocaleTimeString("he-IL", { hour: "2-digit", minute: "2-digit" })}</time><em>{periodLabels[meal.period || "snack"]}</em></span>
          ...
          {state.foods?.length > 0 && <button className="food-library-tile" onClick={() => setFoodLibraryOpen(true)}><span>▦</span><div><strong>ספריית המאכלים</strong><small>{state.foods.length} מאכלים פרטיים ומשותפים · צפייה, עריכה והסרה</small></div><b>←</b></button>}
        </div>
        <div className="side-stack">
          <section className="panel insights-panel"><header><div><p className="eyebrow">לפי המצב היומי</p><h2>המלצות להמשך היום</h2></div><button className="insights-more" onClick={openInsights}>מגמות</button></header><div className="daily-insights">{dailyInsights.map((item) => <article key={item.title}><i>{item.icon}</i><div><strong>{item.title}</strong><small>{item.text}</small></div></article>)}</div><footer className="tracking-actions"><button onClick={() => setActivityOpen(true)}>＋ פעילות</button><button onClick={openInsights}>שבוע וחודש</button></footer></section>
      ...
      {adminLoginOpen && (
        <div className="modal-layer">
          ...
          <form className="settings-modal admin-login" onSubmit={loginAdmin}>
            ...
            <div className="field-stack"><label>סיסמת Admin<input type="password" minLength={10} value={adminPassword} onChange={(e) => setAdminPassword(e.target.value)} autoComplete={state.adminConfigured ? "current-password" : "new-password"} /></label></div>
            {error && <p className="form-error">{error}</p>}
            <footer><button type="button" onClick={() => setAdminLoginOpen(false)}>ביטול</button><button className="primary" disabled={busy || adminPassword.length < 10}>{busy ? "מתחבר…" : state.adminConfigured ? "כניסה" : "צור ADMIN"}</button></footer>
          </form>
        </div>
      )}
        ...
        <div className="modal-layer">
          ...
          <section className={`settings-modal admin-center admin-tab-${adminTab}`}>
            ...
            <section className="admin-users" id="admin-users">
              ...
              <form className="new-user-form" onSubmit={createUser}><h4>יצירת משתמש חדש</h4><div className="settings-grid"><label>שם<input value={newUser.name} onChange={(e) => setNewUser({ ...newUser, name: e.target.value })} /></label><label>אימייל<input type="email" value={newUser.email} onChange={(e) => setNewUser({ ...newUser, email: e.target.value })} /></label><label className="wide">סיסמה זמנית<input type="password" minLength={10} value={newUser.password} onChange={(e) => setNewUser({ ...newUser, password: e.target.value })} /></label></div><button className="primary" disabled={busy || !newUser.name || !newUser.email || newUser.password.length < 10}>צור משתמש</button></form>
            </section>
      ...
      {quickAddOpen && <div className="modal-layer"><button className="backdrop" onClick={() => setQuickAddOpen(false)} /><section className="settings-modal quick-add-modal"><header><div><p className="eyebrow">הוספה מהירה</p><h2>{quickCategory ? "מה להוסיף?" : "בחר קטגוריה"}</h2></div><button onClick={() => setQuickAddOpen(false)}>×</button></header>{!quickCategory ? <div className="category-grid"><button onClick={() => openManualMeal()}><span className="manual-meal-art">🍽</span><strong>ארוחה ידנית</strong><small>AI יחשב לפי התיאור שלך</small></button><button onClick={() => setVoiceOpen(true)}><span className="manual-meal-art">🎙️</span><strong>הקלט ארוחה</strong><small>AI יתמלל ויציג לאישור</small></button><button onClick={() => setQuickCategory("vegetables")}><img src="category-vegetables-v1.png" alt="מבחר ירקות" /><strong>ירקות</strong><small>טריים, מבושלים וסלט</small></button><button onClick={() => setQuickCategory("fruits")}><img src="category-fruits-v1.png" alt="מבחר פירות" /><strong>פירות</strong><small>מנה נפוצה בלחיצה</small></button><button onClick={() => setQuickCategory("drinks")}><img src="category-drinks-v1.png" alt="מבחר משקאות" /><strong>משקאות</strong><small>חמים, קלים, יין ובירה</small></button></div> : <><button className="category-back" onClick={() => setQuickCategory("")}>→ חזרה לקטגוריות</button><div className={`quick-food-grid ${quickCategory}`}>{quickFoods[quickCategory].map((item, index) => <button key={`${item.name}-${item.portion}`} onClick={() => selectQuickFood(item)}><span className="food-sprite" style={foodSpriteStyle(quickCategory, index)} /><strong>{item.name}</strong><small>{item.portion}</small><b>{item.kcal} kcal</b></button>)}{(state.foods || []).filter((food) => food.category === quickCategory).map((food) => <button key={food.id} onClick={() => selectQuickFood({ ...food, portion: "מנה אישית", icon: "🍽" })}>{food.image ? <img src={food.image} alt={food.name} /> : <span>🍽</span>}<strong>{food.name}</strong><small>מהמאגר האישי</small><b>{food.kcal} kcal</b></button>)}<button className="add-food-tile" onClick={() => { setSaveToLibrary(true); setGenerateFoodArtwork(true); openManualMeal(quickCategory); }}><span>＋</span><strong>הוסף פריט אחר</strong><small>הגדר סוג, ערכים ותמונה</small></button></div></>}</section></div>}
      {voiceOpen && <div className="modal-layer"><button className="backdrop" onClick={() => !recording && !busy && setVoiceOpen(false)} /><section className="settings-modal voice-modal"><header><div><p className="eyebrow">הוספה ידנית בקול</p><h2>ספר לי מה אכלת</h2></div><button disabled={recording || busy} onClick={() => setVoiceOpen(false)}>×</button></header><button className={recording ? "voice-record recording" : "voice-record"} onClick={recording ? stopVoiceRecording : startVoiceRecording} disabled={busy}><span>{busy ? "◌" : recording ? "■" : "🎙️"}</span><strong>{busy ? `מעבד… ${voiceProcessingSeconds} שנ׳` : recording ? `עצור ושלח · ${voiceSeconds}/60` : "התחל הקלטה"}</strong></button><p className="voice-status" aria-live="polite">{voiceProcessingSeconds > 15 && busy ? "מנתח פריטים, כמויות וערכים תזונתיים…" : voiceStatus}</p><small className="voice-privacy">קובץ הקול משמש לתמלול בלבד ואינו נשמר. לפני הוספה תוכל לערוך כל פריט וכמות.</small></section></div>}
      {profileOpen && <div className="modal-layer"><button className="backdrop" onClick={() => setProfileOpen(false)} /><form className="settings-modal profile-modal" onSubmit={saveProfile}><header><div><p className="eyebrow">החשבון שלי</p><h2>פרטים אישיים</h2></div><button type="button" onClick={() => setProfileOpen(false)}>×</button></header><div className="profile-photo"><button type="button" onClick={() => avatarInput.current?.click()}>{profileForm.avatar ? <img src={profileForm.avatar} alt="תמונת פרופיל" /> : <span>{state.owner.name[0]}</span>}<small>החלף תמונה</small></button><input ref={avatarInput} className="camera-input" type="file" accept="image/jpeg,image/png,image/webp" onChange={loadAvatar} /><div><strong>{state.owner.name}</strong><small>{state.owner.email}</small><button type="button" onClick={switchUser}>החלף משתמש / יציאה</button></div></div><div className="settings-grid"><label>שם<input value={profileForm.name || ""} onChange={(e) => setProfileForm({ ...profileForm, name: e.target.value })} /></label><label>גיל<input type="number" min="14" max="120" value={profileForm.age || ""} onChange={(e) => setProfileForm({ ...profileForm, age: Number(e.target.value) })} /></label><label>גובה (ס״מ)<input type="number" min="100" max="250" value={profileForm.height || ""} onChange={(e) => setProfileForm({ ...profileForm, height: Number(e.target.value) })} /></label><label>משקל נוכחי (ק״ג)<input type="number" min="25" max="350" step="0.1" value={weightValue || ""} onChange={(e) => setWeightValue(Number(e.target.value))} /></label><label>משקל יעד<input type="number" min="25" max="350" step="0.1" value={profileForm.targetWeight || ""} onChange={(e) => setProfileForm({ ...profileForm, targetWeight: Number(e.target.value) })} /></label><label>רמת פעילות<select value={profileForm.activity || "light"} onChange={(e) => setProfileForm({ ...profileForm, activity: e.target.value })}><option value="low">רוב היום בישיבה</option><option value="light">קצת בתנועה</option><option value="active">פעיל</option><option value="very">פעיל מאוד</option></select></label><label className="wide">מגבלות והעדפות<input value={profileForm.restrictions || ""} onChange={(e) => setProfileForm({ ...profileForm, restrictions: e.target.value })} placeholder="רגישויות, אלרגיות או מזונות שנמנעים מהם" /></label></div><div className="profile-metrics"><span>BMI <b>{profile.caloriePlan?.bmi}</b></span><span>משקל נוכחי <b>{latestWeight} ק״ג</b></span><span>יעד <b>{profile.targetWeight} ק״ג</b></span><span>מגמה <b>{weightEntries.length > 1 ? `${weightChange > 0 ? "+" : ""}${weightChange} ק״ג` : "אין עדיין"}</b></span></div><button className="profile-sharing" type="button" onClick={() => { setProfileOpen(false); setPartnerOpen(true); }}><span>♡</span><div><strong>שיתוף ומעקב משותף</strong><small>הזמנת בן או בת זוג וניהול הרשאות השיתוף</small></div><b>‹</b></button><button className="profile-sharing" type="button" onClick={() => setDark(!dark)}><span>{dark ? "☀" : "◐"}</span><div><strong>ערכת תצוגה</strong><small>{dark ? "ממשק כהה — לחץ למעבר לבהיר" : "ממשק בהיר — לחץ למעבר לכהה"}</small></div><b>‹</b></button><button className="profile-sharing" type="button" onClick={openTrash}><span>♲</span><div><strong>פריטים שנמחקו</strong><small>שחזור ארוחות ופעילויות למשך 30 יום</small></div><b>‹</b></button><a className="profile-export" href="api/export" download>הורד את הנתונים שלי</a><button className="profile-export" type="button" onClick={deleteMyData}>מחק לצמיתות את הנתונים והחשבון שלי</button><footer><button type="button" onClick={switchUser}>החלף משתמש</button><button className="primary" disabled={busy}>שמור ועדכן יעדים</button></footer></form></div>}
      {foodLibraryOpen && <div className="modal-layer"><button className="backdrop" onClick={() => setFoodLibraryOpen(false)} /><section className="settings-modal food-library-modal"><header><div><p className="eyebrow">המאגר האישי</p><h2>ספריית המאכלים</h2></div><button onClick={() => setFoodLibraryOpen(false)}>×</button></header><div className="catalog-list">{state.foods.map((food) => <article key={food.id}>{food.image ? <img src={food.image} alt="" /> : <span>🍽</span>}<div><strong>{food.name}</strong><small>{food.kcal} kcal · {food.visibility === "shared" ? "משותף" : "פרטי"}</small></div><button onClick={() => { useCatalogFood(food); setFoodLibraryOpen(false); }}>הוסף</button>{food.ownerId === state.currentUser.id && <><button onClick={() => setEditingFood({ ...food })}>ערוך</button><button className="danger" onClick={() => removeCatalogFood(food)}>הסר</button></>}</article>)}</div></section></div>}
      {editingFood && <div className="modal-layer modal-nested"><button className="backdrop" onClick={() => setEditingFood(null)} /><form className="settings-modal compact-modal" onSubmit={saveEditedFood}><header><div><p className="eyebrow">ספריית המאכלים</p><h2>עריכת מאכל</h2></div><button type="button" onClick={() => setEditingFood(null)}>×</button></header><div className="settings-grid"><label className="wide">שם<input value={editingFood.name} onChange={(e) => setEditingFood({ ...editingFood, name: e.target.value })} /></label><label>קלוריות<input type="number" min="1" value={editingFood.kcal} onChange={(e) => setEditingFood({ ...editingFood, kcal: Number(e.target.value) })} /></label><label>חלבון<input type="number" min="0" value={editingFood.protein} onChange={(e) => setEditingFood({ ...editingFood, protein: Number(e.target.value) })} /></label><label>פחמימות<input type="number" min="0" value={editingFood.carbs} onChange={(e) => setEditingFood({ ...editingFood, carbs: Number(e.target.value) })} /></label><label>שומן<input type="number" min="0" value={editingFood.fat} onChange={(e) => setEditingFood({ ...editingFood, fat: Number(e.target.value) })} /></label><label className="wide">הרשאה<select value={editingFood.visibility} onChange={(e) => setEditingFood({ ...editingFood, visibility: e.target.value })}><option value="private">פרטי</option><option value="shared">משותף</option></select></label></div><footer><button type="button" onClick={() => setEditingFood(null)}>ביטול</button><button className="primary" disabled={busy}>שמור שינויים</button></footer></form></div>}
      {historyOpen && <div className="modal-layer"><button className="backdrop" onClick={() => setHistoryOpen(false)} /><section className="settings-modal history-modal"><header><div><p className="eyebrow">יומן תזונה</p><h2>ציר הזמן שלי</h2></div><button onClick={() => setHistoryOpen(false)}>×</button></header><div className="history-days">{[...(state.history || []), state.today].sort((a, b) => b.date.localeCompare(a.date)).map((day) => {
        const totals = day.meals.reduce((sum: any, meal: any) => ({ kcal: sum.kcal + Number(meal.kcal || 0), protein: sum.protein + Number(meal.protein || 0), carbs: sum.carbs + Number(meal.carbs || 0), fat: sum.fat + Number(meal.fat || 0) }), { kcal: 0, protein: 0, carbs: 0, fat: 0 });
        const goals = { kcal: Number(profile.calories || 0), protein: Number(profile.protein || 0), carbs: Number(profile.carbs || 0), fat: Number(profile.fat || 0) };
        const statuses = { kcal: goalStatus(totals.kcal, goals.kcal), protein: goalStatus(totals.protein, goals.protein), carbs: goalStatus(totals.carbs, goals.carbs), fat: goalStatus(totals.fat, goals.fat) };
        return <details key={day.date} open={day.date === state.today.date}><summary><div><strong>{new Date(`${day.date}T12:00:00`).toLocaleDateString("he-IL", { weekday: "long", day: "numeric", month: "long" })}</strong><small>{day.meals.length} ארוחות · {day.waterMl || 0} מ״ל מים</small></div><span className={`history-calories ${statuses.kcal.className}`}><b>{totals.kcal.toLocaleString()} / {goals.kcal.toLocaleString()}</b><small>kcal · {statuses.kcal.label}</small></span></summary><div className="history-summary"><span className={statuses.protein.className}>חלבון <b>{totals.protein} / {goals.protein}g</b><small>{statuses.protein.label}</small></span><span className={statuses.carbs.className}>פחמימות <b>{totals.carbs} / {goals.carbs}g</b><small>{statuses.carbs.label}</small></span><span className={statuses.fat.className}>שומן <b>{totals.fat} / {goals.fat}g</b><small>{statuses.fat.label}</small></span></div><section className="history-timeline">{[...day.meals].sort((a: any, b: any) => String(a.time).localeCompare(String(b.time))).map((meal: any) => <article className={`timeline-${meal.period || "snack"}`} key={meal.id}><time>{new Date(meal.time).toLocaleTimeString("he-IL", { hour: "2-digit", minute: "2-digit" })}</time><i />{meal.image ? <img src={meal.image} alt="" /> : <span className="timeline-icon">🍽</span>}<div><em>{periodLabels[meal.period || "snack"]}</em><strong>{meal.name}</strong><small>{meal.protein}g חלבון · {meal.carbs}g פחמימות · {meal.fat}g שומן</small></div><b>{meal.kcal}<small> kcal</small></b></article>)}</section></details>;
      })}</div></section></div>}
      {activityOpen && <div className="modal-layer"><button className="backdrop" onClick={() => setActivityOpen(false)} /><form className="settings-modal compact-modal" onSubmit={addActivity}><header><div><p className="eyebrow">מעקב יומי</p><h2>הוספת פעילות</h2></div><button type="button" onClick={() => setActivityOpen(false)}>×</button></header><div className="settings-grid"><label>סוג פעילות<select value={activityForm.type} onChange={(e) => setActivityForm({ ...activityForm, type: e.target.value })}><option>הליכה</option><option>ריצה</option><option>אימון כוח</option><option>רכיבה</option><option>שחייה</option><option>פעילות אחרת</option></select></label><label>משך בדקות<input type="number" min="0" max="600" value={activityForm.minutes} onChange={(e) => setActivityForm({ ...activityForm, minutes: Number(e.target.value) })} /></label><label>צעדים<input type="number" min="0" value={activityForm.steps} onChange={(e) => setActivityForm({ ...activityForm, steps: Number(e.target.value) })} /></label><label>מרחק בק״מ<input type="number" min="0" step="0.1" value={activityForm.distanceKm} onChange={(e) => setActivityForm({ ...activityForm, distanceKm: Number(e.target.value) })} /></label><label className="wide">קלוריות פעילות — אופציונלי<input type="number" min="0" value={activityForm.activeCalories} onChange={(e) => setActivityForm({ ...activityForm, activeCalories: Number(e.target.value) })} /></label></div><p className="modal-help">קלוריות פעילות מוצגות בנפרד ואינן מתווספות אוטומטית לתקציב האכילה.</p><footer><button type="button" onClick={() => setActivityOpen(false)}>ביטול</button><button className="primary" disabled={busy}>שמור פעילות</button></footer></form></div>}
      {insightsOpen && <div className="modal-layer"><button className="backdrop" onClick={() => setInsightsOpen(false)} /><section className="settings-modal insights-modal"><header><div><p className="eyebrow">Day · Week · Month</p><h2>מגמות ותובנות</h2></div><button onClick={() => setInsightsOpen(false)}>×</button></header>{!insightsData ? <p className="modal-help">מחשב מגמות…</p> : <><div className="trend-summary"><article><small>ציון שבועי</small><strong>{insightsData.summary.weeklyScore || "—"}</strong><span>{insightsData.summary.previousWeeklyScore ? `${insightsData.summary.weeklyScore >= insightsData.summary.previousWeeklyScore ? "↑" : "↓"} משבוע קודם` : "נדרשים עוד ימים"}</span></article><article><small>ממוצע קלוריות</small><strong>{insightsData.summary.averageCalories}</strong><span>ליום, 7 ימים</span></article><article><small>חלבון ממוצע</small><strong>{insightsData.summary.averageProtein}g</strong><span>ליום</span></article><article><small>פעילות</small><strong>{insightsData.summary.activeMinutes}</strong><span>דקות השבוע</span></article></div><p className="trend-narrative">{insightsData.narrative}</p><div className="score-chart">{insightsData.daily.slice(-14).map((day: any) => <div key={day.date} title={`${day.date}: ${day.score}`}><i style={{ height: `${Math.max(6, day.score)}%` }} /><small>{new Date(`${day.date}T12:00:00`).toLocaleDateString("he-IL", { day: "numeric" })}</small></div>)}</div><div className="trend-legend"><span><i className="good" /> 80–100 מאוזן</span><span><i className="attention" /> 55–79 דורש תשומת לב</span><span><i className="low" /> מתחת ל־55</span></div></>}</section></div>}
      {trashOpen && <div className="modal-layer"><button className="backdrop" onClick={() => setTrashOpen(false)} /><section className="settings-modal compact-modal"><header><div><p className="eyebrow">Recycle Bin</p><h2>פריטים שנמחקו</h2></div><button onClick={() => setTrashOpen(false)}>×</button></header><p className="modal-help">הפריטים נשמרים למשך 30 יום לפני מחיקה קבועה.</p><div className="trash-list">{trashItems.length ? trashItems.map((item) => <article key={item.id}><div><strong>{item.data.name || item.data.type || "פריט"}</strong><small>{item.type === "meal" ? "ארוחה" : "פעילות"} · {new Date(item.deletedAt).toLocaleDateString("he-IL")}</small></div><button onClick={() => restoreTrash(item.id)}>שחזר</button></article>) : <p>אין פריטים שנמחקו.</p>}</div></section></div>}
      ...
      {mealOpen && (
        <div className="modal-layer">
          ...
          <form className="settings-modal meal-modal" onSubmit={addMeal}>
            ...
            {manualAiMode && mealItems.length === 0 && <section className="manual-ai-entry"><label>{catalogOnly ? `איזה ${foodCategory === "fruits" ? "פרי" : foodCategory === "vegetables" ? "ירק" : "משקה"} להוסיף?` : "מה אכלת?"}<textarea rows={catalogOnly ? 2 : 3} value={manualDescription} onChange={(e) => setManualDescription(e.target.value)} placeholder={catalogOnly ? "רשום שם ותיאור קצר, למשל: מנגו בינוני" : "למשל: שתי חתיכות פרגית, כוס אורז וסלט קטן"} /></label><button type="button" onClick={analyzeManualDescription} disabled={busy || !manualDescription.trim()}>{busy ? "מחשב…" : catalogOnly ? "צור תמונה וחשב ערכים" : "חשב אוטומטית עם AI"}</button><small>{catalogOnly ? "לאחר החישוב אפשר לבדוק ולתקן את הערכים לפני שמירה בגלריה." : "לאחר החישוב יוצגו כל הפריטים, הכמויות וההנחות לעריכה ולאישור."}</small></section>}
            <div className="settings-grid">
              ...
              {!catalogOnly && <label className="wide">מתי אכלתי?<input type="datetime-local" value={mealDateTime} max={(() => { const date = new Date(); date.setMinutes(date.getMinutes() - date.getTimezoneOffset()); return date.toISOString().slice(0, 16); })()} onChange={(e) => setMealDateTime(e.target.value)} /><small>אפשר לבחור שעה או יום קודמים; הארוחה תמוקם אוטומטית במקום הנכון בהיסטוריה.</small></label>}
            ...
            <section className="library-options"><label><input type="checkbox" checked={saveToLibrary} onChange={(e) => setSaveToLibrary(e.target.checked)} /> שמור בספריית המאכלים שלי</label>{saveToLibrary && <div><label>סוג<select value={foodCategory} onChange={(e) => setFoodCategory(e.target.value)}><option value="meals">ארוחות</option><option value="vegetables">ירקות</option><option value="fruits">פירות</option><option value="drinks">משקאות</option></select></label><label><input type="radio" name="food-visibility" checked={foodVisibility === "private"} onChange={() => setFoodVisibility("private")} /> פרטי — רק אני</label><label><input type="radio" name="food-visibility" checked={foodVisibility === "shared"} onChange={() => setFoodVisibility("shared")} /> משותף — כל המשתמשים</label><button type="button" onClick={() => foodImageInput.current?.click()}>▧ טען תמונה</button><input ref={foodImageInput} className="camera-input" type="file" accept="image/jpeg,image/png,image/webp" onChange={loadFoodImage} /><label><input type="checkbox" checked={generateFoodArtwork} onChange={(e) => setGenerateFoodArtwork(e.target.checked)} /> אם אין תמונה מתאימה, צור באמצעות AI</label></div>}</section>
            <footer>
              <button type="button" onClick={() => setMealOpen(false)}>
                ביטול
              </button>
              ...
              >
                {catalogOnly ? "שמור בגלריה" : mealItems.length ? "אישור, חישוב והוספה" : "שמור ארוחה"}
              </button>
            </footer>
          </form>
        </div>
...
function Login({ values, setValues, submit, busy, error, adminConfigured, adminPassword, setAdminPassword, setupAdmin }: any) {
  return <main className="onboarding-shell" dir="rtl"><header><img src="/caloreazi-wordmark-transparent.png" alt="CALOREAZI" /></header><section className="onboarding-card login-card"><p className="eyebrow">ברוך הבא</p><h1>{adminConfigured ? "כניסה לחשבון" : "הגדרת ADMIN"}</h1><p>{adminConfigured ? "הנתונים והיעדים שלך זמינים רק לאחר התחברות." : "התקנה חדשה: הגדר סיסמת מנהל כדי להמשיך."}</p>{adminConfigured ? <form onSubmit={submit}><div className="field-stack"><label>אימייל או שם משתמש<input type="text" value={values.login} onChange={(e) => setValues({ ...values, login: e.target.value })} autoComplete="username" /></label><label>סיסמה<input type="password" value={values.password} onChange={(e) => setValues({ ...values, password: e.target.value })} autoComplete="current-password" /></label></div>{error && <p className="form-error">{error}</p>}<footer><button className="primary" disabled={busy || !values.login || !values.password}>{busy ? "מתחבר…" : "כניסה"}</button></footer></form> : <form onSubmit={setupAdmin}><div className="field-stack"><label>סיסמת Admin חדשה<input type="password" minLength={10} value={adminPassword} onChange={(e) => setAdminPassword(e.target.value)} autoComplete="new-password" /></label></div>{error && <p className="form-error">{error}</p>}<footer><button className="primary" disabled={busy || adminPassword.length < 10}>צור ADMIN</button></footer></form>}</section><p className="medical-note">יצירת משתמש חדש מתבצעת על ידי מנהל המערכת.</p></main>;
}
  ...
  const screens = [
    <>
      ...
      <div className="field-stack">
        ...
        {bootstrap && <label>סיסמת Admin <small>לפחות 10 תווים</small><input type="password" minLength={10} value={values.adminPassword} onChange={(e) => setValues({ ...values, adminPassword: e.target.value })} autoComplete="new-password" /></label>}
      </div>
    </>,
    <>
    ...
    <main className="onboarding-shell" dir="rtl">
      ...
      <section className="onboarding-card">
        ...
        <footer>
          ...
          <button
            className="primary"
            disabled={busy || (step === 0 && (!values.name.trim() || (bootstrap && (!values.email.includes("@") || values.adminPassword.length < 10))))}
            onClick={() =>
