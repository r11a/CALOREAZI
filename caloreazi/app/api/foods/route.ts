...
export async function POST(request: Request) {
  ...
  const food = { id: crypto.randomUUID(), ownerId: session.userId, ownerName: state.users.find((item) => item.id === session.userId)?.name || "", visibility: body.visibility === "shared" ? "shared" : "private", name: name.slice(0, 120), kcal: Math.round(Number(body.kcal)), protein: Math.max(0, Math.round(Number(body.protein) || 0)), carbs: Math.max(0, Math.round(Number(body.carbs) || 0)), fat: Math.max(0, Math.round(Number(body.fat) || 0)), items: Array.isArray(body.items) ? body.items.slice(0, 30) : [], image, createdAt: new Date().toISOString() };
