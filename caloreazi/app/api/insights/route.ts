...
export async function GET(request: Request) {
  ...
  return Response.json({ daily, sugar: { enabled: data.profile.diabetesStatus && data.profile.diabetesStatus !== "none", today: daily.at(-1)?.sugarCoverage ? daily.at(-1)?.sugar : null, weeklyAverage: sugarRecent.length ? average(sugarRecent, (item) => item.sugar) : null, monthlyAverage: sugarMonth.length ? average(sugarMonth, (item) => item.sugar) : null, coverage: average(sugarMonth, (item) => item.sugarCoverage), days: daily.slice(-30).map((item) => ({ date: item.date, sugar: item.sugar, coverage: item.sugarCoverage })) }, summary: { weeklyScore: average(recent, (item) => item.score), previousWeeklyScore: average(previous, (item) => item.score), averageCalories: average(recent, (item) => item.totals.kcal), averageProtein: average(recent, (item) => item.totals.protein), averageCarbs: average(recent, (item) => item.totals.carbs), averageFat: average(recent, (item) => item.totals.fat), averageWater: average(recent, (item) => item.waterMl), weightChange, steps, activeMinutes: minutes, trackedDays: tracked.length, targetCompliance: tracked.length ? Math.round(withinTarget / tracked.length * 100) : 0, topMeal: topMeal?.name || "—" }, recommendation, narrative: weightChange ? `מגמת המשקל ב־30 הימים האחרונים היא ${weightChange > 0 ? "עלייה" : "ירידה"} של ${Math.abs(weightChange)} ק״ג.` : "נדרשות לפחות שתי מדידות משקל כדי לזהות מגמה." });
}
