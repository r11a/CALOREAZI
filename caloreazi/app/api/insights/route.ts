...
export async function GET(request: Request) {
  ...
  const daily = days.map((day) => { const sugar = day.meals.reduce((sum, meal) => sum + Number(meal.sugar || 0), 0); const trackedMeals = day.meals.filter((meal) => Number(meal.sugarTrackedItems || 0) > 0).length; return { date: day.date, waterMl: Number(day.waterMl || 0), meals: day.meals.length, sugar: Math.round(sugar * 10) / 10, sugarCoverage: day.meals.length ? Math.round(trackedMeals / day.meals.length * 100) : 0, ...calculateDayScore(day, data.profile, data.activity) }; });
