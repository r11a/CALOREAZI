...
export async function POST(request: Request) {
  ...
  const jobId = crypto.randomUUID(); await updateState((latest) => { latest.analysisJobs = latest.analysisJobs || []; latest.analysisJobs.push({ id: jobId, clientId, userId: session.userId, kind: "meal_photo", status: "pending", provider: visionRole.provider, model: visionRole.model, attemptCount: 0, input: { imageDataUrl: String(imageDataUrl), recognitionHint }, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }); latest.analysisJobs = latest.analysisJobs.slice(-500); return latest; });
