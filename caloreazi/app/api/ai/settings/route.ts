...
export async function PUT(request: Request) {
  ...
  const state = await updateState(async (state) => {
    ...
    state.ai = { ...state.ai, provider, model: coach.id, roles: { coach: { provider, model: coach.id, fallbackModel: coachFallback }, vision: { provider, model: vision.id, fallbackModel: visionFallback }, image: { provider, model: image.id, fallbackModel: imageFallback } }, inputCost: coach.inputCost, outputCost: coach.outputCost, monthlyBudget: economyMode ? Math.min(2, Math.max(0.1, Number(body.monthlyBudget) || 2)) : Math.max(0, Number(body.monthlyBudget) || 0), softLimit: Math.min(100, Math.max(1, Number(body.softLimit) || 70)), hardLimit: true, economyMode, autoGenerateMealImages: !economyMode && body.autoGenerateMealImages === true, cloudTtsEnabled: !economyMode && body.cloudTtsEnabled === true };
