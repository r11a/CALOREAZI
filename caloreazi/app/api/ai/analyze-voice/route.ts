...
function parseItems(text: string) {
  ...
  const items = Array.isArray(data.items) ? data.items.slice(0, 20).map((item: any) => ({ name: String(item.name || "").trim().slice(0, 80), grams: number(item.grams, 3000), quantity: Math.max(.1, Math.min(50, Number(item.quantity) || 1)), unit: String(item.unit || "מנה").trim().slice(0, 30), kcalPerUnit: number(item.kcalPerUnit, 2500), kcalPer100: number(item.kcalPer100, 1000), proteinPer100: number(item.proteinPer100, 200), carbsPer100: number(item.carbsPer100, 200), fatPer100: number(item.fatPer100, 200) })).filter((item: any) => item.name && item.grams > 0) : [];
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await call({ apiKey, model: role.model, instructions: "אתה מנתח תזונה זהיר. פרק את תיאור הארוחה לפריטים נפרדים. מספר כמו שתי חתיכות הוא quantity=2 והמשקל הוא לפריט אחד. מספר קלוריות מפורש שנאמר הוא עובדה מחייבת ונשמר ב-kcalPerUnit, אלא אם המשתמש אמר לבדוק, לאשר או שאינו בטוח. kcalPer100 מיועד רק לערך ל-100 גרם. אל תנחש ודאות כשכמות חסרה. החזר JSON תקין בלבד.", input: `תמלול: ${transcript}\nהחזר בדיוק: {"name":"שם ארוחה בעברית","items":[{"name":"שם פריט","grams":150,"quantity":1,"unit":"חתיכה","kcalPerUnit":0,"kcalPer100":100,"proteinPer100":10,"carbsPer100":12,"fatPer100":3}],"confidence":"low|medium|high","explanation":"הנחות קצרות"}` });
    const parsed = parseItems(result.text); const explicit = applyExplicitCalorieFacts(transcript, parsed.items); const analysis = { ...parsed, items: explicit.items, explicitCaloriesEnforced: explicit.enforced }; const cost = estimateCost({ inputTokens: result.usage.inputTokens, outputTokens: result.usage.outputTokens, inputCostPerMillion: selectedModel?.inputCost || state.ai.inputCost, outputCostPerMillion: selectedModel?.outputCost || state.ai.outputCost });
