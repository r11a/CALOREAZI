...
export async function PUT(request: Request) {
  ...
  const { voice, provider, style } = await request.json();
  const coachVoice = selectedVoice(voice);
  const coachVoiceStyle: VoiceStyle = style === "clear" ? "clear" : "warm";
  ...
  await updateState((latest) => {
    ...
    if (data.profile) { data.profile.coachVoice = coachVoice; data.profile.coachVoiceStyle = coachVoiceStyle; data.profile.coachVoiceProvider = coachVoiceProvider; }
    return latest;
  });
  return Response.json({ coachVoice, coachVoiceStyle, coachVoiceProvider });
}
...
export async function POST(request: Request) {
  ...
  const delivery = `${direction}. דבר כמו מאמן אישי אנושי, בונה וסבלני, בעברית טבעית ועכשווית ובקצב שיחה מעט זריז. בלי הטפה, דרמה או התלהבות מלאכותית. הגה מספרים ככמות שלמה ולא ספרה אחר ספרה. אל תקריא סימני Markdown. טקסט: ${text}`;
  ...
  if (role.provider === "gemini") {
    const response = await fetch("https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-tts:generateContent", {
      ...
      body: JSON.stringify({
        ...
        generationConfig: { responseModalities: ["AUDIO"], speechConfig: { voiceConfig: { prebuiltVoiceConfig: { voiceName: voice === "female" ? (style === "clear" ? "Aoede" : "Sulafat") : (style === "clear" ? "Charon" : "Gacrux") } } } },
      }),
    });
  ...
  const response = await fetch("https://api.openai.com/v1/audio/speech", {
    ...
    body: JSON.stringify({
      ...
      voice: voice === "female" ? (style === "clear" ? "nova" : "coral") : (style === "clear" ? "echo" : "onyx"),
      input: text,
