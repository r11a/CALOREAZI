...
export async function DELETE(request: Request) {
  ...
  if (!user || !(await verifyPassword(String(body.password || ""), user.password))) return Response.json({ error: "הסיסמה אינה נכונה" }, { status: 403 });
  ...
  const state = await updateState((latest) => { const data = ensureUserData(latest, session.userId); const day = data.today.date === date ? data.today : data.history.find((item) => item.date === date); if (!day) return latest;
    ...
    if (kind === "water") { const event = (day.waterEvents || []).find((item) => item.id === id); if (event) { latest.trash.push({ id: crypto.randomUUID(), userId: session.userId, type: "water", data: { ...event, date }, deletedAt: new Date().toISOString() }); day.waterEvents = day.waterEvents.filter((item) => item.id !== id); day.waterMl = Math.max(0, day.waterEvents.reduce((sum, item) => sum + Number(item.amount || 0), 0)); removed = true; } }
