...
export async function POST(request: Request) {
  ...
  if (owned.meal.image) return Response.json({ ...userView(state, session.userId, session.role === "admin"), imageCompleted: false, imageSource: "existing" });
  ...
  const latest = await updateState((current) => { const target = findOwnedMeal(current, session.userId, id)?.meal; if (target && !target.image) { target.image = image; target.media = media || catalogMatch?.media || null; target.imageSource = imageSource; } return current; });
