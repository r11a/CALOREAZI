...
export async function POST(request: Request) {
  ...
  const state = await updateState((latest) => { latest.userData[session.userId].today.meals.push({ id: crypto.randomUUID(), name, kcal, protein: Math.max(0, Number(body.protein) || 0), carbs: Math.max(0, Number(body.carbs) || 0), fat: Math.max(0, Number(body.fat) || 0), time: new Date().toISOString() }); return latest; });
...
export async function DELETE(request: Request) {
  ...
  const state = await updateState((latest) => { const today = latest.userData[session.userId].today; today.meals = today.meals.filter((meal) => meal.id !== id); return latest; });
  return Response.json(userView(state, session.userId, session.role === "admin"));
}
