...
export async function POST(request: Request) {
  ...
  const { amount = 250, recordedAt, localDate } = await request.json(); const eventTime = Number.isFinite(Date.parse(String(recordedAt || ""))) ? new Date(recordedAt).toISOString() : new Date().toISOString();
  const state = await updateState((latest) => { const data = ensureUserData(latest, session.userId); const eventDate = /^\d{4}-\d{2}-\d{2}$/.test(String(localDate || "")) ? String(localDate) : entryDateFor(data, eventTime); let day = data.today.date === eventDate ? data.today : data.history.find((item) => item.date === eventDate); if (!day) { day = { date: eventDate, waterMl: 0, waterEvents: [], meals: [] }; data.history.push(day); data.history.sort((a, b) => a.date.localeCompare(b.date)); } const delta = Number(amount || 0); day.waterMl = Math.max(0, Number(day.waterMl || 0) + delta); day.waterEvents = Array.isArray(day.waterEvents) ? day.waterEvents : []; if (delta > 0) day.waterEvents.push({ id: crypto.randomUUID(), amount: delta, time: eventTime }); else if (delta < 0) { let remaining = Math.abs(delta); while (remaining > 0 && day.waterEvents.length) { const last = day.waterEvents.at(-1); if (Number(last.amount) <= remaining) { remaining -= Number(last.amount); day.waterEvents.pop(); } else { last.amount = Number(last.amount) - remaining; remaining = 0; } } } if (mutationKey) { latest.systemSettings.mutationReceipts = [...(latest.systemSettings.mutationReceipts || []), receipt].slice(-2000); } return latest; });
