...
import { applyFoodCorrections } from "./food-learning.js";
...
const produceSourceIds = new Set(["tomato", "cucumber", "lettuce", "apple", "banana", "orange", "mango", "plum", "melon", "grapefruit", "watermelon", "kiwi", "peach"]);
export function normalizePhotoPortions(items = []) {
  const normalized = items.map((item) => {
    const grams = Math.max(1, Number(item.grams) || 1); const quantity = Math.max(.1, Number(item.quantity) || 1); const sourceId = String(item.nutritionSource?.sourceId || "");
    return produceSourceIds.has(sourceId) && grams * quantity > 600 ? { ...item, grams: Math.round(600 / quantity), portionAdjusted: true } : item;
  });
  const produceTotal = normalized.filter((item) => produceSourceIds.has(String(item.nutritionSource?.sourceId || ""))).reduce((sum, item) => sum + Number(item.grams || 0) * Number(item.quantity || 1), 0);
  if (produceTotal <= 700) return normalized;
  const scale = 700 / produceTotal;
  return normalized.map((item) => produceSourceIds.has(String(item.nutritionSource?.sourceId || "")) ? { ...item, grams: Math.max(1, Math.round(Number(item.grams || 0) * scale)), portionAdjusted: true } : item);
}
...
export async function processNextMealAnalysis() {
  ...
  try {
    ...
    const cameraCalibration = userData.profile?.cameraCalibration || {}; const visualReference = cameraCalibration.reference === "plate" ? `המשתמש הגדיר צלחת ייחוס בקוטר ${Number(cameraCalibration.plateDiameterCm || 26)} ס״מ. השתמש בה רק אם צלחת מלאה אכן נראית בתמונה.` : cameraCalibration.reference === "card" ? "אם נראה בתמונה כרטיס תקני, ניתן להשתמש בו כרמז לקנה מידה." : "אין אובייקט ייחוס מאומת בתמונה.";
    const recognitionHint = String(job.input?.recognitionHint || "").trim().slice(0, 300);
    ...
    for (const candidateRole of candidates) { try { role = candidateRole; selectedModel = findModel(role.provider, role.model); const call = role.provider === "gemini" ? generateGeminiCoachReply : generateOpenAiCoachReply; result = await call({ apiKey: await decryptSecret(fresh.ai.encryptedKey), model: role.model, instructions: `אתה מנתח חזותי זהיר. מנה את כל הפריטים והחתיכות ובדוק שלא איחדת סוגי מזון. הערך רק מזון שנראה. grams הוא משקל יחידה אחת ו-quantity מספר היחידות. כשיש כמה חתיכות מאותו מזון, הערך משקל של חתיכה אחת בנפרד והשתמש בכמות כמכפיל; אל תייחס את משקל המנה כולה לכל רכיב. ירקות חתוכים מחושבים רק לפי הנפח הנראה שלהם. searchNameEn הוא שם אנגלי תמציתי לחיפוש במסד תזונה רשמי. אם אין קנה מידה אמין, confidence אינו יכול להיות high גם אם סוגי המזון ברורים, כי המשקל עדיין משוער. אם הזיהוי אינו ברור הורד confidence וציין חלופות. אל תטען שמשקל מתמונה הוא מדויק. ${visualReference} רמז אופציונלי מהמשתמש: ${recognitionHint || "לא נמסר"}. השתמש בו רק כהקשר; אם הוא סותר בבירור את התמונה, ציין זאת ואל תכפה אותו. אל תחזיר קלוריות או ערכים תזונתיים; מנוע נפרד מחשב אותם. תיקוני עבר: ${cameraCalibration.useLearnedCorrections === false ? "השימוש בתיקונים כבוי" : calibration || "אין"}. מאכלים מוכרים: ${knownFoods || "אין"}. החזר JSON בלבד.`, input: '{"name":"שם ארוחה בעברית","items":[{"name":"שם פריט ספציפי","searchNameEn":"plain cooked food name","grams":150,"quantity":1,"unit":"חתיכה"}],"confidence":"low|medium|high","explanation":"מה נראה והנחות כמות"}', imageDataUrl: job.input.imageDataUrl }); break; } catch (error) { lastError = error; } }
    if (!result) throw lastError || new Error("לא נמצא מודל זמין לניתוח הארוחה");
    const vision = parseVisionResult(result.text); const nutrition = await enrichVisionItemsAuthoritative(vision.items); const learnedItems = cameraCalibration.useLearnedCorrections === false ? nutrition.items : applyFoodCorrections(nutrition.items, userData.foodCalibration || []); const normalizedItems = normalizePhotoPortions(learnedItems); const adjustedPortions = normalizedItems.some((item) => item.portionAdjusted); const confidence = vision.confidence === "high" && !cameraCalibration.reference ? "medium" : vision.confidence; const cost = estimateCost({ inputTokens: result.usage.inputTokens, outputTokens: result.usage.outputTokens, inputCostPerMillion: selectedModel?.inputCost || fresh.ai.inputCost, outputCostPerMillion: selectedModel?.outputCost || fresh.ai.outputCost });
    const response = { ...vision, confidence, items: normalizedItems, nutritionStatus: nutrition.nutritionStatus, unmatchedNutritionItems: nutrition.unmatched, quantityEstimated: !cameraCalibration.reference, explanation: `${vision.explanation}${nutrition.unmatched ? ` ${nutrition.unmatched} פריטים דורשים התאמה למקור תזונתי לפני אישור.` : " הערכים חושבו ממקור תזונתי נפרד מהזיהוי החזותי."}${adjustedPortions ? " הערכת כמות חריגה הוגבלה לטווח סביר לצלחת מצולמת." : ""}`.trim(), usage: { ...result.usage, estimatedCost: cost } };
    await updateState((latest) => { const target = latest.analysisJobs.find((item) => item.id === job.id); if (target?.status === "cancelled") return latest; target.status = "needs_confirmation"; target.confidence = confidence; target.result = response; delete target.input; target.updatedAt = new Date().toISOString(); latest.aiUsage.push({ id: crypto.randomUUID(), month: new Date().toISOString().slice(0, 7), at: new Date().toISOString(), userId: job.userId, feature: "meal_photo", provider: role.provider, model: role.model, ...result.usage, cost }); return latest; });
  } catch (error) {
    await updateState((latest) => { const target = latest.analysisJobs.find((item) => item.id === job.id); if (!target || target.status === "cancelled") return latest; target.status = "failed"; target.errorMessage = error instanceof Error ? error.message : "ניתוח התמונה נכשל"; target.nextAttemptAt = Number(target.attemptCount || 0) < 3 ? new Date(Date.now() + 2 ** target.attemptCount * 1000).toISOString() : null; if (!target.nextAttemptAt) delete target.input; target.updatedAt = new Date().toISOString(); return latest; });
  }
  return true;
}
