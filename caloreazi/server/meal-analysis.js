...
import { aiRole, findModel } from "./ai/models.js";
import { enrichVisionItems } from "./nutrition-catalog.js";
...
export function parseVisionResult(text) {
  ...
  const items = Array.isArray(data.items) ? data.items.slice(0, 20).map((item) => ({ name: String(item.name || "").trim().slice(0, 80), grams: number(item.grams, 3000), quantity: Math.max(.1, Math.min(50, Number(item.quantity) || 1)), unit: String(item.unit || "מנה").trim().slice(0, 30) })).filter((item) => item.name && item.grams > 0) : [];
...
export async function processNextMealAnalysis() {
  ...
  try {
    const role = aiRole(fresh.ai, "vision"); const selectedModel = findModel(role.provider, role.model); const call = role.provider === "gemini" ? generateGeminiCoachReply : generateOpenAiCoachReply;
    ...
    const result = await call({ apiKey: await decryptSecret(fresh.ai.encryptedKey), model: role.model, instructions: `אתה מנתח חזותי זהיר. מנה את כל הפריטים והחתיכות ובדוק שלא איחדת סוגי מזון. הערך רק מזון שנראה. grams הוא משקל יחידה אחת ו-quantity מספר היחידות. אם הזיהוי אינו ברור הורד confidence וציין חלופות. אל תחזיר קלוריות או ערכים תזונתיים; מנוע נפרד מחשב אותם. תיקוני עבר: ${calibration || "אין"}. מאכלים מוכרים: ${knownFoods || "אין"}. החזר JSON בלבד.`, input: '{"name":"שם ארוחה בעברית","items":[{"name":"שם פריט ספציפי","grams":150,"quantity":1,"unit":"חתיכה"}],"confidence":"low|medium|high","explanation":"מה נראה והנחות כמות"}', imageDataUrl: job.input.imageDataUrl });
    const vision = parseVisionResult(result.text); const nutrition = enrichVisionItems(vision.items); const cost = estimateCost({ inputTokens: result.usage.inputTokens, outputTokens: result.usage.outputTokens, inputCostPerMillion: selectedModel?.inputCost || fresh.ai.inputCost, outputCostPerMillion: selectedModel?.outputCost || fresh.ai.outputCost });
