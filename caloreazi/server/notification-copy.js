...
const copy = {
  ...
  coach: [["הצעד הבא שלך", ({ name, advice }) => `${name}, ${advice}`], ["מה כדאי עכשיו?", ({ name, advice }) => `${name}, ${advice}`], ["כיוון קטן להמשך היום", ({ name, advice }) => `${name}, ${advice}`], ["המלצה אישית", ({ name, advice }) => `${name}, ${advice}`], ["מבט מהמאמן", ({ name, advice }) => `${name}, ${advice}`]],
  insight: [["תובנת היום", ({ name, insight }) => `${name}, ${insight}`], ["מבט על היום", ({ name, insight }) => `${name}, ${insight}`]],
  progress: [["סיכום ביניים", ({ name, calories, target, focus }) => `${name}, נרשמו ${calories} מתוך ${target} קלוריות. ${focus}`], ["איפה אתה עומד עכשיו?", ({ name, calories, target, focus }) => `${name}, עד עכשיו: ${calories}/${target} קלוריות. ${focus}`], ["מבט קצר על היום", ({ name, calories, target, focus }) => `${name}, נרשמו עד כה ${calories} קלוריות מתוך ${target}. ${focus}`]],
...
export const notificationTestContexts = {
  ...
  progress: { calories: 920, target: 1950, focus: "בארוחה הבאה כדאי לשלב ירקות ומקור חלבון." },
