...
import { entryDateFor } from "../../local-date.js";
...
export function restoreOwnedMeal(state, userId, meal) { const data = ensureUserData(state, userId); const date = /^\d{4}-\d{2}-\d{2}$/.test(String(meal.logicalDate || "")) ? meal.logicalDate : entryDateFor(data, meal.time); const day = data.today.date === date ? data.today : (data.history.find((item) => item.date === date) || (() => { const created = { date, waterMl: 0, meals: [] }; data.history.push(created); return created; })()); day.meals.push(meal); day.meals.sort((a, b) => String(a.time).localeCompare(String(b.time))); return meal; }
