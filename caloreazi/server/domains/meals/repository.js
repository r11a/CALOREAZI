...
export function restoreOwnedMeal(state, userId, meal) { const data = ensureUserData(state, userId); const date = new Intl.DateTimeFormat("en-CA", { timeZone: process.env.TZ || "Asia/Jerusalem", year: "numeric", month: "2-digit", day: "2-digit" }).format(new Date(meal.time)); const day = data.today.date === date ? data.today : (data.history.find((item) => item.date === date) || (() => { const created = { date, waterMl: 0, meals: [] }; data.history.push(created); return created; })()); day.meals.push(meal); day.meals.sort((a, b) => String(a.time).localeCompare(String(b.time))); return meal; }
