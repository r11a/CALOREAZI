...
export function hydrateDatabaseState(raw, now = new Date()) {
  ...
  for (const row of raw.days || []) { const date = String(row.payload?.date || ""); if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) continue; const data = getData(row.userId); const today = data.profile?.dayBoundaryMode === "manual" && /^\d{4}-\d{2}-\d{2}$/.test(String(data.profile.activeDayDate || "")) ? data.profile.activeDayDate : localDateAt(now, userTimeZone(data)); const day = { ...row.payload, date, meals: [] }; if (day.date === today) data.today = day; else data.history.push(day); }
  for (const row of raw.meals || []) { const data = getData(row.userId); const today = data.profile?.dayBoundaryMode === "manual" && /^\d{4}-\d{2}-\d{2}$/.test(String(data.profile.activeDayDate || "")) ? data.profile.activeDayDate : localDateAt(now, userTimeZone(data)); const candidate = String(row.localDate || ""); const localDate = /^\d{4}-\d{2}-\d{2}$/.test(candidate) ? candidate : localDateAt(row.payload?.time || now, userTimeZone(data)); let day = localDate === today ? data.today : data.history.find((item) => item.date === localDate); if (!day || !day.date) { day = { date: localDate, waterMl: Number(day?.waterMl || 0), meals: day?.meals || [] }; if (localDate === today) data.today = day; else data.history.push(day); } day.meals.push(row.payload); }
...
export async function replaceDatabaseState(state, expectedRevision = null) {
  ...
  for (const user of users) {
    ...
    for (const day of days) for (const meal of day.meals || []) {
      ...
      statements.push(`INSERT INTO meals(id,user_id,daily_record_id,media_id,client_request_id,name,period,source,occurred_at,kcal,protein_g,carbs_g,fat_g,score,confidence,transcript,payload) SELECT '${mealId}','${user.id}',id,${mediaId ? `'${mediaId}'` : "NULL"},${textSql(meal.clientRequestId)},${textSql(meal.name || "Meal")},${textSql(["breakfast","lunch","dinner","snack"].includes(meal.period) ? meal.period : "snack")},${textSql(source)},${dateSql(meal.time)},${numberSql(meal.kcal)},${numberSql(meal.protein)},${numberSql(meal.carbs)},${numberSql(meal.fat)},${meal.score == null ? "NULL" : numberSql(meal.score)},${meal.confidence == null ? "NULL" : numberSql(meal.confidence)},${textSql(meal.transcript)},${jsonSql(meal)} FROM daily_records WHERE user_id='${user.id}' AND local_date=${textSql(day.date)};`);
