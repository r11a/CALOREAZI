...
export async function replaceDatabaseState(state, expectedRevision = null) {
  ...
  for (const user of users) {
    ...
    for (const day of days) statements.push(`INSERT INTO daily_records(user_id,local_date,water_ml,payload) VALUES('${user.id}',${textSql(day.date)},${numberSql(day.waterMl)},${jsonSql({ date: day.date, waterMl: Number(day.waterMl || 0) })}) ON CONFLICT(user_id,local_date) DO UPDATE SET water_ml=EXCLUDED.water_ml,payload=EXCLUDED.payload;`);
