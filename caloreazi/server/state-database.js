...
export async function insertDatabaseMeal({ userId, localDate, timeZone, meal, analysisJobId = "", calibrations = [] }) {
  ...
  if (analysisJobId) statements.push(`UPDATE analysis_jobs SET status='completed',completed_at=NOW(),updated_at=NOW(),payload=jsonb_set(jsonb_set(payload,'{status}','\"completed\"'::jsonb),'{mealId}',to_jsonb('${meal.id}'::text)) WHERE id='${analysisJobId}' AND user_id='${userId}';`);
