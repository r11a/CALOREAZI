...
export async function transcribeMealAudio({ provider, apiKey, model, audioDataUrl }) {
  ...
  if (provider === "gemini") {
    const { payload } = await requestAi(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(apiKey)}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: "תמלל את ההקלטה בדיוק לעברית. החזר רק את התמלול, ללא הסבר." }, { inlineData: { mimeType, data: encoded } }] }], generationConfig: { maxOutputTokens: 700, thinkingConfig: { thinkingLevel: "low" } } }),
    }, { provider: "Gemini", timeoutMs: 35_000 });
    return (payload.candidates?.[0]?.content?.parts || []).filter((part) => !part.thought).map((part) => part.text || "").join("").trim();
  }
