...
- כפתור הוספת הארוחה בסרגל התחתון סוגר באופן גורף כל חלון או תהליך פתוח ומחזיר למסך המקורות הראשי.

## 1.23.2

- כפתור יציאה צף וקבוע מעל המצלמה וסורק הברקוד ב־PWA, כולל התאמה לאזור הבטוח באייפון.

## 1.23.1

- שמונת מקורות הוספת הארוחה מוצגים באריחים גדולים וברורים בפריסה של שתי עמודות וארבע שורות.
- גובה האריחים מותאם אוטומטית למסכים נמוכים כדי לשמור את כל מסך ההוספה ללא גלילה אנכית.

## 1.23.0

- חלון הוספת הארוחה עוצב מחדש כמסך אחד: חיפוש, שמונה מקורות הוספה בשתי שורות וקרוסלת ארוחות אחרונות או נפוצות.
- ירקות ופירות אוחדו לקטגוריה אחת, וכל זרימות ההוספה הקיימות נשמרו מאחורי אייקונים חדשים ואחידים.
- מצלמת הארוחה וסריקת הברקוד מציגות כפתור חזרה ברור וקבוע.

## 1.22.1

- ממשק ה־PWA בנייד נעול לרוחב המכשיר ומאפשר גלילה אנכית בלבד, ללא זום בצביטה או גלילה אופקית.
- פתיחת שלוש המלצות שומרת על מיקום אופקי יציב וכל תוכן ההמלצות נעטף בתוך רוחב המסך.

## 1.22.0

- צילום, דיבור, ברקוד וחיפוש מגיעים לאותו מסך אישור עם שם, כמות, קלוריות, מקור ואמינות.
- לאחר הוספת ארוחה, שתייה או פעילות במצב מקוון ניתן לבטל מיד במשך 15 שניות, והמדדים מתעדכנים בהתאם.
- בדיקות הסבירות מזהות סתירה בין קלוריות למאקרו, כמות חריגה ורכיבים כפולים לפני השמירה.
- נוסף סיכום יומי קצר עם פעולה אחת להצגת שלוש הצעות המתאימות לשעה ולפערים של היום.
- מסך המגמות מציג תובנת שבוע אישית המחושבת מקומית מנתוני המשתמש, ללא עלות AI נוספת.
- מרכז הסנכרון מציג מצב שמירה, פעולות ממתינות וכשלים עם אפשרות ניסיון חוזר.
- מרכז ADMIN מציג מדדי איכות: זמן הוספה, אישור זיהויים, תיקונים, אמינות, כפילויות ועלות AI לארוחה.

## 1.21.0

- תיקוני משתמש בארוחה מלמדים כעת את המערכת גם שם, משקל, כמות, קלוריות ומאקרו ומשמשים אוטומטית בזיהוי הבא של אותו מאכל.
- ערך מפורש בהזנה חדשה נשאר בעדיפות עליונה; למידה קודמת אינה דורסת מידע חדש שמסר המשתמש.
- נוספה בדיקת כפילות בטווח של 10 דקות: לחיצה ראשונה מזהירה ולחיצה נוספת מאשרת שמדובר במנה נוספת.
- בדף הבית מוצגות עד ארבע ארוחות רגילות לפי היסטוריית 14 הימים האחרונים והשעה הנוכחית, להוספה בלחיצה אחת.
- הוספה מהירה נתמכת גם Offline, עם מזהה פעולה למניעת שמירה כפולה בסנכרון.

## 1.20.10

- בחירת משקה במרכז השתייה מוסיפה בר ריק בערך 0 ואינה רושמת צריכה עד ללחיצה מפורשת על `+`.
- לחיצה על `−` מבטלת את מנת המשקה האחרונה בלבד ומסירה יחד איתה את הארוחה והערכים התזונתיים שנוצרו מההוספה.
- משקאות שזוהו מתוך ארוחה היסטורית מוגנים מהפחתה דרך הבר, כדי לא לנתק את מאזן הנוזלים מהארוחה המקורית.
- הוספה והפחתה פועלות באופן עקבי גם במצב Offline ונשמרות לסנכרון לפי הסדר.

## 1.20.9

- משקאות שכבר נשמרו בארוחות ובהיסטוריה מזוהים אוטומטית ונכנסים רטרואקטיבית למאזן הנוזלים, ללא הזנה חוזרת וללא כפילויות.
- הזיהוי ההיסטורי תומך בעברית בקפה, תה, חלב, שוקו, קקאו, מיץ, סודה, משקה קל, יין ומים ומשתמש בכמות השמורה כשקיימת.
- מגמות השתייה כוללות מעבר יומי, שבועי וחודשי עבור מיכל המשקאות וגרף שעות השתייה.
- שמירת העדפות מרכז השתייה אינה יכולה עוד לאפס בטעות את הכמות היומית.

## 1.20.8

- כפתור הוספת המשקה עוצב מחדש כפעולת מערכת ברורה עם אייקון, Heebo וניגודיות מלאה.
- בממשק בהיר הכפתור משתמש במשטח כתום עדין; בממשק כהה הוא מקבל משטח גרניט כהה וטקסט בהיר.

## 1.20.7

- נוסף כפתור ברור להוספת משקה ישירות ממרכז השתייה; לאחר הבחירה והשמירה מופיע מיד בר נפרד למשקה.
- נוספו קפה שחור, קפה עם חלב, שוקו, קקאו, תה ירוק, תה צמחים, משקה קל ממותק ויין.
- ניתן ליצור משקה מותאם עם גודל מנה וערכי קלוריות ומאקרו, הנשמרים ומשפיעים אוטומטית על כל מדדי היום.
- מיכל השכבות עוצב מחדש כמיכל זכוכית נקי, ללא צורת הקנקן המעוותת.

## 1.20.6

- מרכז השתייה מציג מים ומשקאות נוספים לבחירה, עם פס התקדמות וכפתורי הוספה והפחתה נפרדים לכל משקה.
- קנקן יומי מונפש בדף הבית וקנקן מגמות ל־30 יום מציגים את הרכב השתייה בשכבות צבעוניות.
- כל משקה תורם למיכל הנוזלים לפי מקדם שקוף; קלוריות ומאקרו מתווספים אוטומטית לסיכום היום בלי הזנה כפולה ובלי שורה כפולה בציר הזמן.
- הפעולה המלאה נשמרת גם Offline ומסתנכרנת עם החזרה לרשת.

## 1.20.5

- נוסף מצב חיסכון קשיח שמגביל את תקציב ה-AI ל־$2 בחודש ומונע הפעלת קול ענן ויצירת תמונות אוטומטית.
- תמונות ארוחות מושלמות כעת מתמונות אמיתיות בקטלוג וב־Open Food Facts, נשמרות במטמון ל־30 יום ומשמשות מחדש ללא קריאת AI.
- כשאין תמונה אמינה מוצג איור קטגוריה מקומי; שמירת ארוחה לעולם אינה מתעכבת או נכשלת בגלל חיפוש תמונה.
- נוספו דגמי Gemini Flash-Lite חסכוניים לבחירה בהגדרות המנהל.

## 1.20.4

- לחיצה על כוכב בתוצאת החישוב שומרת מיד את הארוחה במועדפים, גם בלי להוסיף אותה ליומן.
- לחיצה נוספת מסירה את המועדף, והכוכב משקף אם ארוחה בשם זה כבר שמורה.
- השמירה וההסרה המיידיות פועלות גם Offline ומציגות פידבק ברור ליד הכוכב.

## 1.20.3

- סימון הכוכב שומר כעת את הארוחה בפועל במועדפים בעת האישור, בלי תלות ביום שבו הארוחה נשמרה.
- שמירה למועדפים פועלת גם עבור ארוחות היסטוריות ובמצב Offline ומסתנכרנת כשהחיבור חוזר.
- הודעת האישור מציינת במפורש שהארוחה נשמרה גם במועדפים.

## 1.20.2

- עגבניות שרי, עגבניות ברבים ועגבניות חצויות מותאמות כעת לערך הטרי המהימן של 18 קלוריות ל־100 גרם.
- התאמת מאגר חיצונית נפסלת אם מזון טרי הותאם בטעות לגרסה מיובשת, מטוגנת, מסוכרת או מעובדת.
- נוספה חסימת שמירה לערך אנרגטי בלתי סביר של פרי או ירק טרי, עם הנחיה לחישוב מחדש.
- מטמון הזיהוי עודכן כדי שתוצאות שגויות מהמסלול הקודם לא ימוחזרו לאחר העדכון.

## 1.20.1

- אותה תמונה עם אותו רמז משתמשת כעת בתוצאת הניתוח הקודמת, כדי למנוע קפיצות אקראיות בין חישובים חוזרים.
- זיהוי צילום ללא קנה מידה אינו מקבל עוד ציון ודאות גבוה, ומוצג טווח קלורי גלוי שמפריד בין זיהוי המזון לבין הערכת המשקל.
- נוספה הגנת סבירות שמונעת מכל ירק או פרי מזוהה לקבל בטעות את משקל הצלחת כולה.
- פעולות האישור והביטול נמצאות בתחתית התוכן ואינן צפות מעל כרטיס התוצאה בטלפון.
- שמירה במועדפים מוצגת ככוכב ריק שהופך למלא לאחר בחירה.

## 1.20.0

- כל מקורות הוספת הארוחה מובילים למסך תוצאה אחיד: תמונה כשזמינה, קלוריות בולטות, הסבר חישוב ופעולות עריכה, חישוב מחדש ושמירה במועדפים.
- נוספו ארבע טבעות מונפשות וברורות עבור אמינות הזיהוי או החישוב, חלבון, פחמימות ושומן ביחס לפער שנותר באותו יום.
- המצלמה נפתחת בתוך האפליקציה עם הנחיית צילום קצרה ושדה הקשר אופציונלי שמשמש את הזיהוי כרמז בלבד.
- כפתור הפעולה המרכזי והכפתור בכותרת משתמשים באייקון ארוחה עם סימן הוספה ופותחים את תפריט מקורות ההוספה המלא.
- יעדים רגועים מוצגים סגורים כברירת מחדל, ונוסף סימון עדין לרכיבים שניתנים להרחבה.
- כוסות מים מוצגות כעת לפי שעה בציר הזמן של „מה אכלת היום”, כולל פעולות שנשמרו במצב Offline.
- חוזקה גלילת מסך תוצאת הארוחה בטלפון כדי שהפעולות והתוכן לא יוסתרו מאחורי סרגל הניווט.

## 1.19.3

- תוקנה גלילת מסך תוצאת זיהוי הארוחה ב־iPhone ובמכשירי מגע, כולל תוכן ארוך ואזור פעולות קבוע.
- ציון הזיהוי מוצג כעת בכרטיס בולט ונפרד מיד לאחר התמונה, עם מספר ברור מתוך 100.
- נוספה טבעת ציון מונפשת, צבע לפי רמת הביטחון והסבר קצר לגבי הפעולה המומלצת.

## 1.19.2

- כל זיהוי ארוחה מתמונה או מקול מציג כעת ציון זיהוי מספרי וברור מתוך 100.
- הציון נשמר עם הארוחה ומוצג גם בחלון הפרטים לאחר ההוספה ובהיסטוריה.
- רמת הציון מסומנת בצבע ובהנחיה קצרה: זיהוי ברור, בדיקת כמויות מומלצת או בדיקה נדרשת.
- ציון הזיהוי מופרד בכוונה מציון אמינות הערכים התזונתיים, כדי שלא להציג ודאות כוזבת לגבי קלוריות וכמויות.

## 1.19.1

- מאגר התזונה הלאומי הישראלי משתתף כעת גם בהשלמת ערכים לאחר זיהוי צילום, ולא רק בחיפוש ידני.
- ההתאמה למאגר הישראלי דורשת שם עברי תואם או קרוב ואינה בוחרת אוטומטית רשומה לא קשורה.
- לכל התאמה ישראלית נשמרים קוד המקור, ערכי 100 גרם ומקור ממשלתי מפורש לציון האמינות.
- נוסף Cache מוגבל של שבעה ימים למקורות המרוחקים כדי לשפר מהירות בלי לצבור זיכרון ללא גבול.
- USDA נשאר גיבוי למזון שלא נמצא בעברית, ו־AI נשאר רק להשלמת פריטים ללא מקור תזונתי.

## 1.19.0

- נוספה שכבת אמינות מלאה לכל ארוחה: ציון, כיסוי נתונים ורמת אמינות לפני האישור.
- כל מרכיב מציג את מקור הערך ואת נוסחת החישוב המדויקת ששימשה לקלוריות.
- נתון מפורש של המשתמש, מאגר ממשלתי, קטלוג מאומת, USDA, ברקוד והערכת AI מקבלים דירוג מקור נפרד ושקוף.
- זיהוי מתמונה ומקול מקבל הפחתת ודאות מכוונת כאשר הכמות לא אושרה בידי המשתמש.
- נוספה בדיקת סתירה בין ערך ליחידה לבין ערך ל־100 גרם כדי למנוע בסיס חישוב כפול או שגוי.
- Benchmark הארוחות מודד כעת הטיית יתר או חסר, אחוז תוצאות בטווח 10% ו־20%, ומזהיר כשאין עדיין מדגם מסחרי מספיק.
- דוח האמינות נשמר עם הארוחה ומתעדכן מחדש לאחר עריכת מרכיבים או כמויות.

## 1.18.30

- ביקורת חישוב מקצה לקצה חיזקה את ההפרדה בין ערכים ליחידה לבין ערכים ל־100 גרם בכל מסלולי השמירה.
- ערכים קלוריים מפורשים אינם נפסלים בגלל מאקרו שהוערך על ידי AI.
- בדיקת סבירות למאכלים צפופי קלוריות נאכפת גם בהזנה ידנית שחושבה באמצעות AI.
- השלמת AI אינה מופעלת מחדש כשכבר קיים ערך מדויק ליחידה.
- כמויות שהוכתבו במילים בעברית, כגון „שתי” או „שלושה עשר”, מומרות לכמות מדויקת לפני החישוב.

## 1.18.29

- ערך קלורי מפורש שהמשתמש כתב או הכתיב נשמר כערך מחייב ליחידה ואינו נדרס על ידי AI.
- בקשה מפורשת לבדוק או לאשר מאפשרת ל־AI להציע חישוב במקום לקבע את המספר שנאמר.
- החישוב מבדיל בין קלוריות ליחידה לבין קלוריות ל־100 גרם ומציג את נוסחת הכמות לפני השמירה.
- נוספה בדיקת סבירות לערכים נמוכים באופן חריג בעוגות, עוגיות, ביסקוויטים וממתקים.

## 1.18.28

- פתיחת הארוחה המאוזנת מחליפה זמנית את מסך המגמות במקום להופיע מאחוריו.
- סגירת פרטי הארוחה מחזירה אוטומטית למסך המגמות.
